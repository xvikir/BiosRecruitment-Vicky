...
Unlike many of the later Ethernaut levels, this level does not require exploiting a vulnerability. Instead, it teaches the basics of smart contract interaction and contract reconnaissance.
...
# PART 2 — Getting the Level Instance
...
# PART 3 — Following the Information Chain

```markdown
## 3. Following the Information Chain

I called the function suggested by the previous response:
...
At this point, I did not initially know whether infoNum was a function or a property that could be accessed directly.

Instead of guessing, I decided to inspect the contract object and its ABI.

---

# PART 4 — Inspecting the Contract ABI

```markdown
## 4. Inspecting the Contract and ABI
...
# PART 5 — Finding the Next Method

```markdown
## 5. Finding `theMethodName()`
...
# PART 8 — Calling `authenticate()`

```markdown
## 8. Authenticating With the Password
